...
A minimal marketplace: sellers list items, buyers place orders. Placing an order writes the `Order`
row, enqueues a payment job, and appends an `OrderPlaced` event — all in one EF Core transaction, no
outbox table.
